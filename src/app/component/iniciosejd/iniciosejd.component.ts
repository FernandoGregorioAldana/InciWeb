...
@Component({
  selector: 'app-iniciosejd',
  templateUrl: './iniciosejd.component.html',
  styleUrls: ['./iniciosejd.component.css']
})
export class IniciosejdComponent {
  
  //Declaramos el formulario que usaremos
  formlog: FormGroup;
  ...
  nos llevara directamente a la pagina de inicio de la plataforma  para el director academico*/
  onbutton(){
    this.userService.login(this.formlog.value)
    .then(() => {
      this.showsucces();
      this.router.navigate(['Iniciojd']);
    })
