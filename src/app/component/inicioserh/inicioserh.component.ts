...
@Component({
  selector: 'app-inicioserh',
  templateUrl: './inicioserh.component.html',
  styleUrls: ['./inicioserh.component.css']
})
export class InicioserhComponent {
  ...
  nos llevara directamente a la pagina de inicio de la plataforma  para recursos humanos*/
  onbutton(){
    this.userService.login(this.formlog.value)
    .then(() => {
      this.showsucces();
      this.router.navigate(['InicioRh']);
    })
